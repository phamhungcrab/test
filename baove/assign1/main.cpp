...
const int MOD = (int) 1e9 + 7, INF = (int) 1e9 + 9, maxn = (int) 2e2 + 6, maxm = (int) 1e5 + 5;
...
int n, c[maxn][maxn], matched[maxn], t, vis[maxn];
vector<int> adj[maxn];
void I()
{
    cin >> n;
...
bool matching(int u)
{
    ...
    for (int v : adj[u]) if(!matched[v] || matching(matched[v]))
    {
        matched[v] = u;
        return 1;
    }
...
bool check(int timer)
{
    ...
    For (i, 1, n) For (j, 1, n)
        if (c[i][j] <= timer) adj[i].pb(j);
    For (i, 1, n) t++, ans += matching(i);
    return ans == n;
}
int O()
{
